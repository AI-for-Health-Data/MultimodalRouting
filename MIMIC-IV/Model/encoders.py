...
def _ensure_2d_mask(mask: Optional[torch.Tensor], B: int, T: int, device) -> torch.Tensor:
    """
    Normalize mask to shape [B, T] with float dtype (1=keep, 0=pad).
    """
    if mask is None:
...
class BEHRTLabEncoder(nn.Module):
    ...
    def _pos(self, T: int, std: float = 0.02) -> torch.Tensor:
        # grow learnable positional embeddings if needed
        if self.pos.size(1) < T:
            extra = torch.randn(1, T - self.pos.size(1), self.pos.size(-1),
                                device=self.pos.device, dtype=self.pos.dtype) * std
            self.pos = nn.Parameter(torch.cat([self.pos, extra], dim=1))
        return self.pos[:, :T, :]

    def encode_seq(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None
...
class BioClinBERTEncoder(nn.Module):
    ...
    def _encode_token_windows_to_cls(self, token_windows: List[Dict[str, torch.Tensor]]) -> torch.Tensor:
        """
        Batch windows -> CLS embeddings.
        Returns [S, hidden] BEFORE any projection to out_dim.
        """
        device = self._device()
    ...
    def encode_chunks(
        ...
        """
        Like encode_seq but expects text is already chunked per patient.
        """
        device = self._device()
        seqs: List[torch.Tensor] = []
        lengths: List[int] = []
...
class ImageEncoder(nn.Module):
    ...
    def forward(
        ...
        if isinstance(x, torch.Tensor):
            ...
            elif x.dim() == 4:  # vectorized batch path
                x = x.to(device)
                h = self.backbone(x)
        ...
        # List[List[Tensor]] -> take last per case
    ...
    def encode_seq(
        ...
        """
        Return a "sequence" view for consistency with other encoders:
          Hpad: [B, 1, D], M: [B, 1]
        """
        device = next(self.parameters()).device
