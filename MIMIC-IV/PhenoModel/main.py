...
def save_heatmap_with_numbers(
    ...
    plt.xlabel("Route" if len(col_names) == 7 else "")
...
def resolve_image_path(p: str, dataset_root: str) -> str:
    ...
    if os.path.isabs(p):
        return p
    image_root = getattr(CFG, "image_root", "") or dataset_root
...
class ICUStayDataset(Dataset):
    ...
    def __init__(self, root: str, split: str = "train"):
        ...
        if self.notes_mode == "text":
            ...
            # any pair (input_ids_* and mask_*) with non-empty lists
            nonempty = np.zeros(len(self.notes), dtype=bool)
            for c_id, c_m in zip(self.input_id_cols, self.attn_mask_cols):
                # cells may be list/ndarray/scalar -> normalize len safely
...
def evaluate_epoch(
    ...
    avg_pa = (act_sum / max(1, num_samples)).numpy()  # [7]
    ...
    if num_samples > 0 and has_routing and rc_sum_mat is not None:
        ...
        avg_effective_mat = avg_rc_mat * avg_pa[:, None]  # [7, K]
    else:
        avg_rc_mat = None
...
def collect_epoch_logits(
    ...
    for xL, mL, notes, imgs, y, dbg in loader:
        ...
        zN = _safe_tensor(_clamp_norm(zN, 20.0), "collect_logits.zN").float()
        zI = _safe_tensor(_clamp_norm(zI, 20.0), "collect_logits.zI").float()
...
def collect_epoch_outputs(
    ...
    for xL, mL, notes, imgs, y, dbg in loader:
        ...
        zN = _safe_tensor(_clamp_norm(zN, 20.0), "collect.zN").float()
        zI = _safe_tensor(_clamp_norm(zI, 20.0), "collect.zI").float()
...
def generate_split_heatmaps_and_tables(
    ...
    # pa_vec: [7], eff_mat: [7,K], rc_mat: [7,K]
    ...
    # Effective weights (Kx7)
    eff_norm, _, _ = save_array_with_versions(
        eff_k7, out_dir, f"{split_name.lower()}_effective_weights_kx7",
        row_names=label_names, col_names=routes,
        print_title=f"[{split_name}] EFFECTIVE WEIGHTS (primary_act × routing_coef) [Kx7] (raw + normalized)"
    )
    save_heatmap_with_numbers(
        ...
        col_names=routes,
        title=f"{split_name} Effective Weights (Kx7) | normalized color, raw numbers",
        out_path=os.path.join(out_dir, f"{split_name.lower()}_effective_weights_kx7.png"),
        fontsize_cell=6,
        fontsize_ticks=9
    )

    # Mean routing coeff (Kx7)
    rc_norm, _, _ = save_array_with_versions(
        rc_k7, out_dir, f"{split_name.lower()}_mean_routing_coeff_kx7",
        row_names=label_names, col_names=routes,
        print_title=f"[{split_name}] MEAN ROUTING COEFFICIENT (per-route, per-phenotype) [Kx7] (raw + normalized)"
    )
    save_heatmap_with_numbers(
        mat_norm=rc_norm,
        mat_raw=rc_k7,
        row_names=label_names,
        col_names=routes,
        title=f"{split_name} Mean Routing Coefficient (Kx7) | normalized color, raw numbers",
        out_path=os.path.join(out_dir, f"{split_name.lower()}_mean_routing_coeff_kx7.png"),
        fontsize_cell=6,
        fontsize_ticks=9
    )
    ...
    return {
        ...
        "effective_kx7": eff_k7.astype(np.float32),
        "routing_coeff_kx7": rc_k7.astype(np.float32),
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for step, (xL, mL, notes, imgs, y, dbg) in enumerate(train_loader):
            ...
            # sanitize + clamp + cast to fp32 for capsule stability
            zL = _safe_tensor(_clamp_norm(zL, max_norm=20.0), "zL(behrt)").float()
            zN = _safe_tensor(_clamp_norm(zN, max_norm=20.0), "zN(bbert)").float()
            zI = _safe_tensor(_clamp_norm(zI, max_norm=20.0), "zI(imgenc)").float()

            if _has_nonfinite(zL, zN, zI):
            ...
            with amp_ctx_caps:
                ...
                if route_dropout_p > 0.0:
                    if torch.rand((), device=DEVICE) < route_dropout_p:
                        drop_idx = int(torch.randint(low=0, high=7, size=(1,), device=DEVICE))
                        route_mask[:, drop_idx] = 0.0
                    if (epoch - start_epoch) < 2 and (torch.rand((), device=DEVICE) < route_dropout_p * 0.5):
                        drop_idx2 = int(torch.randint(low=0, high=7, size=(1,), device=DEVICE))
    ...
    # FINAL HEATMAPS (TRAIN + TEST ONLY) using BEST checkpoint
