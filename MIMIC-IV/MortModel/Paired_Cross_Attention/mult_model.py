...
class MULTModel(nn.Module):
    def __init__(self, orig_d_l, orig_d_n, orig_d_i, d_l, d_n, d_i,
        ...
        # Always build directional bimodal routes (method constructs all ordered pairs)
        ...
        # ---------------------------------------------------------------------
        # UPDATED (to match paragraph):
        # Build trimodal LNI via hierarchical composition of directional pairs:
        #   e_LN = Proj_LN([e_{L<-N}; e_{N<-L}])  -> R^{B x d_l}
        #   e_LI = Proj_LI([e_{L<-I}; e_{I<-L}])  -> R^{B x d_l}
        #   e_NI = Proj_NI([e_{N<-I}; e_{I<-N}])  -> R^{B x d_l}
        #   e_LNI = Proj_T([e_LN; e_LI; e_NI])    -> R^{B x d_l}
        #
        # Directional dims differ (d_l, d_n, d_i). We minimally align N/I -> d_l.
        # ---------------------------------------------------------------------
    ...
    def get_network(self, self_type: str = 'l', layers: int = -1):
        # 1) Respect caller-provided layers; otherwise use self.layers
        n_layers = self.layers if layers == -1 else layers

        # 2) Query modality decides embed_dim / dropout (first char: l/n/i)
        # Works for: l, n, i, ln, nl, li, il, ni, in, lni, ... and also l_only/n_only/i_only
        q = self_type[0]
        if q == 'l':
        ...
        return TransformerEncoder(
            ...
            layers=n_layers,              # <-- IMPORTANT: pass the resolved n_layers
    ...
    def forward_from_encoders(
        ...
        # Unimodal routes come from encoders (preferred). Fallback if None.
