...
# Determinism (must be set before some CUDA ops initialize)
...
def configure_reproducibility(seed: int, deterministic: bool, cudnn_benchmark: bool) -> None:
    """
    Configure RNG seeds + deterministic flags.
    Note: full determinism may still be affected by some CUDA ops / libs.
    """
    if deterministic:
        os.environ.setdefault("CUBLAS_WORKSPACE_CONFIG", ":4096:8")
...
# -----------------------------------------------------------------------------
# Routing definition (must match routing_and_heads / main asserts)
# -----------------------------------------------------------------------------
...
# Canonical task for this repo/run
TASKS: List[str] = ["mortality"]

LABEL_NAMES: List[str] = ["mortality"]
...
class Config:
    ...
    # Logging / debug cadence
    routing_print_every: int = 100  # <-- IMPORTANT: was referenced in env_map but missing in Config
    ...
    # If you want to avoid accidental mismatch with your parquet bins, keep -1 (whole stay).
    # Set explicitly via YAML/env/CLI if you truly want a fixed window.
    structured_seq_len: int = 48     # -1 => keep all time bins (whole stay)
    structured_pos_max_len: int = 2048
    structured_n_feats: int = 76
    structured_layers: int = 2
    structured_heads: int = 8
    # Structured data schema (matches structured_mortality.parquet)
    structured_format: str = "packed"          # "packed" or "long"
    structured_x_col: str = "x_ehr_48h_2h_76"   # packed sequence column
    structured_y_col: str = "y_true"
    structured_split_col: str = "split"
    structured_id_col: str = "stay_id"
    ...
    # No-dropout anti-collapse routing controls (sigmoid gating)
    route_gate_temp: float = 3.0
    route_gate_min: float = 0.05
    route_gate_max: float = 0.95

    # Regularizers (single source of truth: route_* fields)
    ...
    # Backward-compat alias fields (kept for older code/CLI/env; always synced in load_cfg)
    ...
    routing_coef_mode: str = "gate_norm"   # "gate_norm" | "softmax" | "none"
    ...
    task_name: str = "mortality"  # <-- UPDATED default (was "pheno")
...
def amp_autocast_dtype(precision_amp: str) -> Optional[torch.dtype]:
    """
    Returns dtype for autocast, or None to disable autocast.
    """
...
def _coerce_types(d: Dict[str, Any]) -> Dict[str, Any]:
    """
    Light coercion for YAML/JSON/env overrides.
    """
    def conv(x: Any) -> Any:
        if isinstance(x, str):
            s = x.strip()
...
def _sync_routing_aliases(cfg: Config) -> None:
    ...
    # Gate alias support (older code might look for gate_* names)
...
def load_cfg(yaml_path: Optional[str] = None,
    ...
    if "MIMICIV_CFG_JSON" in os.environ:
        try:
            blob = json.loads(os.environ["Attach"])
    ...
    env_map = {
        ...
        "MIMICIV_STRUCT_FORMAT": ("structured_format", str),
        "MIMICIV_STRUCT_X_COL": ("structured_x_col", str),
        "MIMICIV_STRUCT_Y_COL": ("structured_y_col", str),
        "MIMICIV_STRUCT_SPLIT_COL": ("structured_split_col", str),
        "MIMICIV_STRUCT_ID_COL": ("structured_id_col", str),
    ...
    SELECTED_TASK_IDX = TASK2IDX.get(CFG.task_name, 0)  # always 0 for mortality-only
...
def apply_cli_overrides(args: Any) -> None:
    """
    Apply argparse-style CLI overrides on top of already loaded CFG.
    Important: do not use bool("false") (it becomes True). Use _str2bool.
    """
    global CFG
...
def get_label_name(idx: int, label_names: Optional[List[str]] = None) -> str:

    names = label_names if (label_names and len(label_names) > 0) else LABEL_NAMES
    if 0 <= idx < len(names):
