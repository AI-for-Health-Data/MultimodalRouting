...
class CapsuleFC(nn.Module):
    ...
    def forward(
        ...
        if next_capsule_value is None:
            ...
            next_capsule_value = torch.einsum("bnm, bna, namd, bn->bmd", query_key, input, w, current_act)   # [B,N_out,D]
            ...
            if uniform_routing:
                ...
                if next_act is None:
                    next_act = torch.ones(B, self.out_n_capsules, device=input.device, dtype=query_key.dtype)
