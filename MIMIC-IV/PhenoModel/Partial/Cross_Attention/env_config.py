...
class Config:
    ...
    # Data filtering
    require_all_modalities_train: bool = True
    require_all_modalities_eval: bool = False     # allow missing in val/test
    min_modalities_eval: int = 1                  # keep samples with >= this many modalities
    ...
    # Missingness evaluation policy
    eval_missing_policy: str = "natural"          # natural | drop_L | drop_N | drop_I | random_one
    eval_random_drop_p: float = 1.0               # for random_one: probability to drop one modality
...
def load_cfg(yaml_path: Optional[str] = None,
    ...
    env_map = {
        ...
        "MIMICIV_ENTROPY_USE_RC": ("entropy_use_rc", lambda s: str(s).lower() in {"1", "true", "yes"}),
    }
    ...
    # Always keep route_num consistent with ROUTES (10 routes for cross-attn setup)
