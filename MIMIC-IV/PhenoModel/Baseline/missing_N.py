...
def parse_args():
    ...
    ap.add_argument(
        "--finetune_text", action="store_true",
    ...
    # --- TEST / evaluation controls ---
    ap.add_argument(
        "--test_only", action="store_true",
        help="Skip training; load checkpoint (best/resume) and run test scenarios."
    )
    ap.add_argument(
        "--test_scenario", type=str, default="MISSING_N",
        choices=["TRIMODAL", "MISSING_N", "MISSING_I", "ALL"],
        help="Which scenario to evaluate."
    )

    # --- modality-missing ablations (applied to VAL+TEST inside scenario runner) ---
    ap.add_argument(
        "--test_mask_frac_n", type=float, default=1.0,
        help="Fraction of VAL/TEST patients to mask Notes (N) for missing-modality-N eval."
    )
    ap.add_argument(
        "--test_mask_frac_i", type=float, default=1.0,
        help="Fraction of VAL/TEST patients to mask Images (I) for missing-modality-I eval."
    )
    ap.add_argument("--test_mask_seed_n", type=int, default=1337)
    ap.add_argument("--test_mask_seed_i", type=int, default=2337)
...
def _route_indices_containing(letter: str) -> List[int]:
    letter = str(letter).strip().upper()
    return [i for i, r in enumerate(ROUTE_NAMES[:N_ROUTES]) if letter in str(r).upper()]

def _deterministic_mask_from_ids(stay_ids: List[int], frac: float, seed: int) -> torch.Tensor:
    """
    Deterministic per-patient mask using stay_id hashing.
    Returns CPU bool tensor [B].
    """
    frac = float(frac)
    if frac <= 0.0:
        return torch.zeros(len(stay_ids), dtype=torch.bool)
    if frac >= 1.0:
        return torch.ones(len(stay_ids), dtype=torch.bool)

    sid = torch.tensor([int(x) for x in stay_ids], dtype=torch.int64)
    # LCG-ish hash (stable)
    h = (sid * 1103515245 + int(seed)) & 0xFFFFFFFF
    u = h.to(torch.float32) / 4294967296.0  # in [0,1)
    return (u < frac)

def _zero_out_encoded_modality(out: Dict[str, torch.Tensor], miss_mask: torch.Tensor) -> Dict[str, torch.Tensor]:
    """
    out: {"seq":[B,T,D], "mask":[B,T], "pool":[B,D]} (on DEVICE)
    miss_mask: bool [B] (CPU or DEVICE)
    """
    if out is None:
        return out
    if miss_mask is None or (not bool(miss_mask.any().item() if isinstance(miss_mask, torch.Tensor) else any(miss_mask))):
        return out

    out2 = dict(out)
    m = miss_mask.to(out2["pool"].device, non_blocking=True)

    if out2.get("seq", None) is not None:
        out2["seq"][m] = 0.0
    if out2.get("pool", None) is not None:
        out2["pool"][m] = 0.0
    if out2.get("mask", None) is not None:
        out2["mask"][m] = 0.0
    return out2

def _apply_route_mask_for_missing(route_mask: torch.Tensor, miss_mask, letter: str) -> torch.Tensor:
    """
    route_mask: [B,R] float32 on DEVICE
    miss_mask : bool-ish [B] (CPU/GPU tensor, numpy, list) -> we coerce to bool [B]
    letter    : 'N' or 'I'
    """
    idxs = _route_indices_containing(letter)
    if route_mask is None or len(idxs) == 0 or miss_mask is None:
        return route_mask

    # --- force boolean mask on same device, shape [B] ---
    m = torch.as_tensor(miss_mask, device=route_mask.device).view(-1).bool()
    B = int(route_mask.size(0))
    if m.numel() != B:
        # be safe if something upstream gave mismatched length
        m = m[:B] if m.numel() > B else torch.cat([m, torch.zeros(B - m.numel(), device=m.device, dtype=torch.bool)])

    if not bool(m.any().item()):
        return route_mask

    rows = torch.nonzero(m, as_tuple=True)[0]  # [n_true]
    cols = torch.tensor(idxs, device=route_mask.device, dtype=torch.long)  # [n_idx]

    # Explicit broadcast to [n_true, n_idx]
    route_mask[rows[:, None], cols[None, :]] = 0.0
    return route_mask
...
def evaluate_epoch(
    ...
    missing_cfg: Optional[Dict[str, Dict[str, Any]]] = None,   # <--- ADD
):
    behrt.eval()
    imgenc.eval()
    ...
    for bidx, (xL, mL, notes, imgs, y, dbg) in enumerate(loader):
        ...
        B = xL.size(0)
        stay_ids = dbg.get("stay_ids", list(range(B)))

        missN = None
        missI = None
        if missing_cfg is not None:
            if "N" in missing_cfg:
                missN = _deterministic_mask_from_ids(
                    stay_ids,
                    missing_cfg["N"].get("frac", 1.0),
                    missing_cfg["N"].get("seed", 1337),
                )
            if "I" in missing_cfg:
                missI = _deterministic_mask_from_ids(
                    stay_ids,
                    missing_cfg["I"].get("frac", 1.0),
                    missing_cfg["I"].get("seed", 2337),
                )

        # ensure on DEVICE and boolean (safe even if helper returns numpy/list)
        if missN is not None:
            missN = torch.as_tensor(missN, device=DEVICE).bool().view(B)
        if missI is not None:
            missI = torch.as_tensor(missI, device=DEVICE).bool().view(B)
        ...
        if missN is not None and missN.any():
            outN = _zero_out_encoded_modality(outN, missN)
        if missI is not None and missI.any():
            outI = _zero_out_encoded_modality(outI, missI)

        with amp_ctx_caps:
            z = {"L": outL, "N": outN, "I": outI}
            route_mask = torch.ones(xL.size(0), N_ROUTES, device=DEVICE, dtype=torch.float32)
            if missN is not None and missN.any():
                route_mask = _apply_route_mask_for_missing(route_mask, missN, "N")
            if missI is not None and missI.any():
                route_mask = _apply_route_mask_for_missing(route_mask, missI, "I")
...
def collect_epoch_outputs(
    ...
    act_temperature: float = 1.0, 
    detach_priors: bool = False,
    missing_cfg: Optional[Dict[str, Dict[str, Any]]] = None, 
):
    behrt.eval()
    imgenc.eval()
    if getattr(bbert, "bert", None) is not None:
...
def generate_split_heatmaps_and_tables(
    ...
    y_true_log, logits, _ = collect_epoch_logits(
        ...
        amp_ctx_enc, amp_ctx_caps, missing_cfg=missing_cfg
    )
...
def main():
    ...
    best_path_default = os.path.join(ckpt_dir, "best.pt")
    ckpt_path = None

    if args.resume and os.path.isfile(args.resume):
        ckpt_path = args.resume
    elif os.path.isfile(best_path_default):
        ckpt_path = best_path_default

    start_epoch = 0
    if ckpt_path is not None:
        print(f"[main] Loading checkpoint: {ckpt_path}")
        start_epoch = load_checkpoint(
            ckpt_path, behrt, bbert, imgenc, mult, route_adapter, projector, cap_head, optimizer
        )
    else:
        print(f"[main] No checkpoint found. Looked for --resume or {best_path_default}")

    if getattr(args, "test_only", False):
        print("[main] --test_only set: skipping training loop.")
        # jump to the "load BEST + run_test_scenario" section
        # easiest: set args.epochs = start_epoch so the for-loop does nothing
    ...
    if os.path.isfile(best_path):
        print("[main] Loading BEST checkpoint for final test scenarios:", best_path)
        _ = load_checkpoint(best_path, behrt, bbert, imgenc, mult, route_adapter, projector, cap_head, optimizer)

    def _print_route_summaries(split_tag: str, rc_mat: Optional[np.ndarray], eff_mat: Optional[np.ndarray], pa_vec: Optional[np.ndarray]):
        if pa_vec is not None:
            print(f"\n[{split_tag}] Primary activations (mean):")
            for i, r in enumerate(ROUTE_NAMES[:N_ROUTES]):
                print(f"  {r:4s}: {float(pa_vec[i]):.4f}")

        if rc_mat is not None:
            print(f"\n[{split_tag}] Mean p(route|phenotype) per route (avg over phenotypes):")
            rc_mean = rc_mat.mean(axis=1)  # [R]
            for i, r in enumerate(ROUTE_NAMES[:N_ROUTES]):
                print(f"  {r:4s}: {float(rc_mean[i]):.4f}")

        if eff_mat is not None:
            print(f"\n[{split_tag}] Mean effective weight per route (avg over phenotypes):")
            eff_mean = eff_mat.mean(axis=1)  # [R]
            for i, r in enumerate(ROUTE_NAMES[:N_ROUTES]):
                print(f"  {r:4s}: {float(eff_mean[i]):.4f}")

    def run_test_scenario(tag: str, missing_cfg: Optional[Dict[str, Dict[str, Any]]]):
        print("\n" + "=" * 140)
        print(f"[SCENARIO] {tag}")
        if missing_cfg is None:
            print("  -> TRIMODAL (no masking)")
        else:
            print(f"  -> missing_cfg={missing_cfg}")
        print("=" * 140)

        scenario_dir = os.path.join(ckpt_dir, "test_scenarios", tag.lower())
        os.makedirs(scenario_dir, exist_ok=True)

        # --------- VAL: collect logits -> fit temperature -> find thresholds ----------
        y_val, logits_val, _ = collect_epoch_logits(
            val_loader, behrt, bbert, imgenc, mult, route_adapter, projector, cap_head,
            amp_ctx_enc, amp_ctx_caps,
            act_temperature=1.0, detach_priors=False,
            missing_cfg=missing_cfg,
        )
        T_star = fit_temperature_scalar_from_val(logits_val, y_val, max_iter=300, lr=0.05)
        print(f"[{tag}] Fitted temperature on VAL: T={T_star:.4f}")

        p_val = sigmoid_np(apply_temperature(logits_val, T_star))
        thr_val, f1_val_thr = grid_search_thresholds(y_val, p_val, n_steps=101)
        save_split_thresholds(thr_val, scenario_dir, split_name="VAL")
        ...
        _print_route_summaries(f"TEST_{tag}", test_rc_mat, test_eff_mat, test_pa)

        y_test, logits_test, _ = collect_epoch_logits(
            test_loader, behrt, bbert, imgenc, mult, route_adapter, projector, cap_head,
            amp_ctx_enc, amp_ctx_caps,
            act_temperature=1.0, detach_priors=False,
            missing_cfg=missing_cfg,
        )
        p_test_cal = sigmoid_np(apply_temperature(logits_test, T_star))
        y_pred = (p_test_cal >= thr_val[np.newaxis, :]).astype(float)

        mt = epoch_metrics(y_test, p_test_cal, y_pred)

        print(
            f"\n[{tag}] TEST (VAL-thresholds, calibrated probs) "
            f"loss={test_loss:.4f} acc={test_acc:.4f} "
            f"MACRO AUROC={mt['AUROC']:.4f} AUPRC={mt['AUPRC']:.4f} "
            f"F1={mt['F1']:.4f} Recall={mt['Recall']:.4f} Precision={mt['Precision']:.4f}"
        )
        print(
            f"[{tag}] TEST MICRO  AUROC={mt['AUROC_micro']:.4f} AUPRC={mt['AUPRC_micro']:.4f} "
            f"Precision={mt['Precision_micro']:.4f} Recall={mt['Recall_micro']:.4f} F1={mt['F1_micro']:.4f}"
        )
        print(f"[{tag}] TEST example-F1={mt['F1_example']:.4f} Hamming={mt['Hamming']:.4f}")
        print(f"[{tag}] TEST Confusion Matrix:\n{mt['CM']}")

        # Per-phenotype CSV
        df_test = pd.DataFrame({
            "phenotype": label_names,
            "AUROC": mt["AUROC_per_label"],
            "AUPRC": mt["AUPRC_per_label"],
            "F1": mt["F1_per_label"],
            "Recall": mt["Recall_per_label"],
            "Precision": mt["Precision_per_label"],
            "Prevalence_TEST": y_test.mean(axis=0),
        }).sort_values("AUROC", ascending=False)

        csv_path = os.path.join(scenario_dir, f"test_per_phenotype_metrics_{tag.lower()}.csv")
        df_test.to_csv(csv_path, index=False, float_format="%.6f")
        print(f"[{tag}] Saved per-phenotype metrics CSV → {csv_path}")

        # Also print per-phenotype (full list)
        print(f"\n[{tag}] Per-phenotype metrics:")
        for _, r in df_test.iterrows():
            print(
                f"  {str(r['phenotype']):60s} "
                f"AUROC={float(r['AUROC']):.4f} "
                f"AUPRC={float(r['AUPRC']):.4f} "
                f"F1={float(r['F1']):.4f} "
                f"Recall={float(r['Recall']):.4f} "
                f"Precision={float(r['Precision']):.4f} "
                f"Prev={float(r['Prevalence_TEST']):.4f}"
            )

        # Save routing/heatmap tables for this scenario on TEST
        print(f"\n[{tag}] Saving routing/heatmap tables...")
        ...
        return {
            ...
            "macro_AUROC": float(mt["AUROC"]),
            "macro_AUPRC": float(mt["AUPRC"]),
            "macro_F1": float(mt["F1"]),
            "macro_Recall": float(mt["Recall"]),
            "macro_Precision": float(mt["Precision"]),
        }

    print("[main] Evaluating BEST checkpoint on TEST scenarios...")

    # Build configs
    missN_cfg = {"N": {"frac": float(args.test_mask_frac_n), "seed": int(args.test_mask_seed_n)}}
    missI_cfg = {"I": {"frac": float(args.test_mask_frac_i), "seed": int(args.test_mask_seed_i)}}

    results = []

    if args.test_scenario == "TRIMODAL":
        results.append(run_test_scenario("TRIMODAL", missing_cfg=None))

    elif args.test_scenario == "MISSING_N":
        results.append(run_test_scenario("MISSING_N", missing_cfg=missN_cfg))

    elif args.test_scenario == "MISSING_I":
        results.append(run_test_scenario("MISSING_I", missing_cfg=missI_cfg))
    ...
    for r in results:
        print(
            f"  {r['tag']:10s} | T={r['T']:.3f} | loss={r['test_loss']:.4f} acc={r['test_acc']:.4f} "
            f"| AUROC={r['macro_AUROC']:.4f} AUPRC={r['macro_AUPRC']:.4f} "
            f"F1={r['macro_F1']:.4f} Recall={r['macro_Recall']:.4f} Precision={r['macro_Precision']:.4f}"
        )
    print("=" * 140)
